/************************************* September 24, 2021 at 9:34:34 AM CDT
*****************************************************************************
*
...
****************************************************************************/
#include "si5351.h"
//	DON'T FORGET TO MAKE THIS CHANGE TO THE LOCAL COPY of si5351.h!!!
...
#ifdef WWV10
const int32_t INITIAL_FREQ  = 10000000L;	//	useful during calibration with WWV
#else
const int32_t INITIAL_FREQ = 7074000L;		// for 40m-10m FT8 USB
#endif
...
#define cSI5351_I2C	0x62	//	current boards use a variant part
...
#define GPIO_B1Filter	7		//	10 meters DO NOT USE: Preenabled
#define GPIO_B2Filter	8		//	15 meters
#define GPIO_B3Filter	9		//	20 meters
#define GPIO_B4Filter	10		//	40 meters
//#define GPIO_B4Filter	27		//	40 meters (old work around for prototype Bug)
...
#define GPIO_notTX		22		//	a low enables the tx mixer
#define GPIO_notRX		23		//	a low enables the rx mixer
...
void enableTXAudio(bool txon, bool digital)
{
	if (txon)
		{
		muteHpVol(true);
		...
		if (digital)
			...
			writeSGTL(CHIP_ANA_CTRL,0x0011);	//	mute all but lineout from ADC/DAC
...
void enableTX(bool txon, bool digital)
{
	enableTXAudio(txon, digital);
	if (txon)
		{
...
void checkKeydown(void)
{
	if (!digitalRead(GPIO_Keyer_KEYDOWN))
		{
		enableTX(true,false);
//		enableTXAudio(true,false);
		setADCVol(getADCVol());
		writeSGTL(CHIP_ANA_CTRL,4);
//		digitalWrite(GPIO_notRX, LOW);
		}
	else
		{
//		enableTXAudio(false,true);
		enableTX(false,false);
		}
}
...
void setupSynthesizers(long correction, uint32_t startFreq, bool calibrate)
{
	...
	si5351.init(SI5351_CRYSTAL_LOAD_0PF, SI5351_XTAL_FREQ, correction);
	if (calibrate)
